	...
	public class DBLoader<T> : IDisposable where T : IDBLoadable
	{
		...
		public DBLoader(String filename)
		{
			...
			try
			{
				...
				sql = new SqliteConnection(GetConnectionString(fullfilename));
				...
				catch (Exception ex)
				{
					...
					// Close the database if it happens to be open (shouldn't be with the above failure)
					sql.Dispose();
					// Move it so that we can take a look at it to see why it died
				...
				foreach (DBTableDesc desc in tables)
				{
					...
					else if (!TableMatchesDescription(sql, desc))
					{
						needsCopy = true;
						break;
					}
				...
				if (needsCopy)
				{
					...
					using (SqliteConnection sql2 = new SqliteConnection(GetConnectionString(newfile)))
					{
						foreach (DBTableDesc table in tables)
							CopyTable(sql, sql2, table);

						// Close the original database so that it can be deleted and replaced
						sql.Dispose();
					}
		...
		{
			using (DataSet set = sql.SelectIntoDataSet("PRAGMA table_info(" + table.Name + ");"))
			{
				Dictionary<string, DBFieldDesc> fieldInfo = new Dictionary<string, DBFieldDesc>();

				// Get a list of the fields in the current version
				foreach (var field in table.Fields)
					fieldInfo.Add(field.Name, field);

				// Compare against the existing database. Validate that each field is accounted for
				// and if there are any extra rows from the existing database, then there isn't a match.
				foreach (DataRow row in set.Tables[0].Rows)
				{
					string name = row.GetField<string>("name");

					if (fieldInfo.ContainsKey(name))
						fieldInfo.Remove(name);
					else if (name != "ID" && name != "OwnerID")
						return false;
				}

				return fieldInfo.Count == 0;
			}
		}
		...
		private void UpdateNextIndexForTableFromDB(string tableName)
		{
			...
			//object result = sql.ExecuteScalar<object>(statement);
